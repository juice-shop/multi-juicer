...
export function createTeamsRouteHandler({ kubernetesApi }) {
  const router = Router();
  ...
  async function joinIfTeamAlreadyExists(req, res, next) {
    ...
    try {
      const { passcodeHash } = await kubernetesApi.getJuiceShopInstanceForTeamname(team);
  ...
  async function checkIfMaxJuiceShopInstancesIsReached(req, res, next) {
    ...
    try {
      const response = await kubernetesApi.getJuiceShopInstances();
  ...
  async function createTeam(req, res) {
    ...
    try {
      ...
      await kubernetesApi.createDeploymentForTeam({ team, passcodeHash: hash });
      await kubernetesApi.createServiceForTeam(team);
  ...
  async function resetPasscode(req, res) {
    ...
    try {
      await kubernetesApi.changePasscodeHashForTeam(req.teamname, hash);
  ...
  async function awaitReadiness(req, res) {
    ...
    try {
      for (let i = 0; i < 180; i++) {
        const { readyReplicas } = await kubernetesApi.getJuiceShopInstanceForTeamname(team);
