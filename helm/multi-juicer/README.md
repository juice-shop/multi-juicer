...
| balancer.logLevel | string | `"info"` | Log level for the balancer service. One of: debug, info, warn, error |
...
| balancer.replicas | int | `3` | Number of replicas of the balancer deployment |
...
| config.juiceShop.deleteInactiveAfter | string | `"24h"` | How long a Juice Shop instance may sit idle (no end-user requests) before the balancer deletes it. Accepts Go duration strings, e.g. "24h", "30m", "90m". |
...
| config.juiceShop.tag | string | `"snapshot"` |  |
...
| imagePullSecrets | list | `[]` | imagePullSecrets used for the balancer image. You'll also need to set `config.juiceShop.imagePullSecrets` to set the imagePullSecrets if you are using a private registry for all images |
