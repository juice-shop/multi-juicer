...
export function createProxyRouteHandler({ kubernetesApi, proxy }) {
  const router = Router();
  ...
  async function checkIfInstanceIsUp(req, res, next) {
    ...
    try {
      const { readyReplicas } = await kubernetesApi.getJuiceShopInstanceForTeamname(teamname);
    ...
    try {
      if (connectionCache.has(teamname)) {
        ...
        if (timeDifference > 10000) {
          ...
          await kubernetesApi.updateLastRequestTimestampForTeam(teamname);
        }
      } else {
        await kubernetesApi.updateLastRequestTimestampForTeam(teamname);
        connectionCache.set(teamname, currentTime);
