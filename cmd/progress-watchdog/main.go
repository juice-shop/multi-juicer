...
func main() {
	progresswatchdog.Logger.Info("Starting ProgressWatchdog")
	...
	progresswatchdog.StartBackgroundSync(clientset, numberWorkers)
	...
	router.HandleFunc("POST /team/{team}/webhook", func(responseWriter http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			progresswatchdog.Logger.Error("failed to get deployment for team received via webhook", "team", team, "error", err)
		}
		...
		if err != nil {
			progresswatchdog.Logger.Error("failed to decode json from juice shop deployment annotation", "error", err)
		}
		...
		if err != nil {
			progresswatchdog.Logger.Error("failed to decode cheat scores from juice shop deployment annotation", "error", err)
			cheatScores = make([]progresswatchdog.CheatScoreEntry, 0)
		}
		...
		for _, status := range challengeStatus {
			if status.Key == webhook.Solution.Challenge {
				progresswatchdog.Logger.Info("Challenge already solved, ignoring webhook", "challenge", webhook.Solution.Challenge, "team", team)
		...
		if err != nil {
			progresswatchdog.Logger.Warn("Failed to parse timestamp, using current time in UTC", "timestamp", webhook.Solution.IssuedOn, "error", err)
		...
		newChallengeStatus := progresswatchdog.ChallengeStatus{
		...
		progresswatchdog.PersistProgress(clientset, team, challengeStatus, cheatScores)

		progresswatchdog.Logger.Info("Received webhook", "team", team, "challenge", webhook.Solution.Challenge)
	...
	progresswatchdog.Logger.Info("Starting web server listening for Solution Webhooks on :8080")
